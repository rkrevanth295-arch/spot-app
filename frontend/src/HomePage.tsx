...
export default function HomePage() {
  ...
  useEffect(() => {
    const debounce = window.setTimeout(fetchSpots, searchQuery.trim() ? 200 : 0);
  ...
  async function fetchSpots() {
    ...
    try {
      ...
      const url = term ? `/spots/search/?q=${encodeURIComponent(term)}` : '/spots/?limit=200';
  ...
  const visibleSpots = selectedCategory === 'All' || searchQuery.trim()
    ? spots
  ...
  return (
    <div className="h-screen bg-[#0B0E11] text-[#F5F5F0] flex flex-col overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <Map selectedCategory={selectedCategory} searchQuery={searchQuery} spots={spots} onPinTap={handlePinTap} userLocation={userLocation} />
      </div>
