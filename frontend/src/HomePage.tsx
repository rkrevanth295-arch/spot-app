...
import { Search, MapPin, MapPinIcon, Zap, X, Sparkles } from 'lucide-react';
...
export default function HomePage() {
  ...
  async function fetchSpots() {
    ...
    try {
      ...
      const url = term
        ? `/spots/search/?q=${encodeURIComponent(term)}`
        : '/spots/?limit=200';
  ...
  // Calculate these from the actual API data.
  const categoryCount = new Set(
    spots.map((spot) => spot.category).filter(Boolean)
  ).size;
    ...
    if ('geolocation' in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setLocating(false);
          setLocationError(
            'Location access is unavailable. Check your browser permissions.'
          );
        },
        {
  ...
  // Pick a random currently visible spot.
  const handleSurpriseMe = () => {
    if (visibleSpots.length === 0) return;

    const randomIndex = Math.floor(
      Math.random() * visibleSpots.length
    );

    setSelectedSpot(visibleSpots[randomIndex]);
  };
    ...
    <div className="h-screen bg-[#0B0E11] text-[#F5F5F0] flex flex-col overflow-hidden">
      ...
      <div className="absolute top-0 left-0 right-0 z-[1000] px-4 pt-4 pointer-events-none">
        <div className="bg-[#151A1F]/80 backdrop-blur-xl border border-[rgba(255,255,255,0.08)] rounded-2xl px-4 py-2.5 flex items-center gap-3 pointer-events-auto">
          ...
          <input
            type="text"
            placeholder="Search spots, neighbourhoods..."
            value={searchQuery}
      ...
      {locationError && (
        <div
          role="status"
          className="absolute top-40 left-4 right-4 z-[1000] rounded-xl border border-[#FF6B4A]/30 bg-[#151A1F]/95 px-3 py-2 text-xs text-[#F5F5F0] shadow-lg"
        >
          {locationError}
        </div>
      )}
        ...
        <div className="flex justify-between items-center pointer-events-auto">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={visibleSpots.length === 0}
              aria-label="Surprise me"
              title="Surprise me"
              className="bg-[#151A1F]/80 backdrop-blur-xl rounded-full w-10 h-10 border border-[rgba(255,255,255,0.08)] flex items-center justify-center disabled:opacity-40"
            >
        ...
        <div className="overflow-x-auto whitespace-nowrap flex gap-2 pb-1 pointer-events-auto">
          ...
          <motion.button
            ...
            className={`px-3.5 py-1.5 rounded-full text-xs whitespace-nowrap transition-all ${
              selectedCategory === 'All'
                ? 'bg-[#FF6B4A] text-white shadow-[0_0_18px_rgba(255,107,74,0.42)]'
                : 'bg-[#151A1F]/80 border border-[rgba(255,255,255,0.08)] text-[#F5F5F0]'
            }`}
          >
            ✦ All
          </motion.button>
      ...
      {/* BOTTOM SHEET */}
      <div
        className="absolute bottom-[calc(3.5rem+env(safe-area-inset-bottom))] left-0 right-0 z-[500] pointer-events-none"
        style={{ height: '70vh' }}
      >
        <motion.div
          animate={{
        ...
        >
          ...
          >
            <div
              className={`w-10 h-1 bg-[#8A8F98]/40 rounded-full ${
                !isExpanded
                  ? 'drag-handle-pulse'
                  : ''
              }`}
          ...
          {/* SPOT LIST */}
          <div
            className="px-5 overflow-y-auto space-y-2.5"
            style={{
          ...
          >
            ...
            <div className="bg-gradient-to-r from-[#FF6B4A]/20 to-[#F5A623]/20 border border-[#FF6B4A]/30 rounded-2xl p-3 mb-2">
              ...
              >
                <h3 className="text-sm font-semibold text-[#FF6B4A]">
                  Browse All Categories
                </h3>

                <p className="text-xs text-[#8A8F98] mt-0.5">
                  {categoryCount} categories · {spotCount} spots
                </p>
              </button>
            </div>
            ...
            {loading ? (
              ...
              >
                {[0, 1, 2].map((index) => (
                  <div
                    key={index}
                    className="h-20 rounded-2xl border border-white/[0.05] bg-[#0B0E11] overflow-hidden"
                  >
              ...
              visibleSpots.map((spot) => (
                ...
                >
                  ...
                  <div className="flex-1 py-2 pr-3">
                    ...
                    <h3 className="font-semibold text-[#F5F5F0] text-xs">
                      {spot.name}
                    </h3>

                    <span className="inline-block mt-0.5 px-1.5 py-0.5 bg-[#FF6B4A]/15 text-[#FF6B4A] text-[9px] rounded-full">
                      {spot.category}
                    </span>

                    <p className="text-[#8A8F98] text-[11px] mt-1 line-clamp-1">
                      {spot.description}
                    </p>
